-- ============================================================
-- ChemicalLaboratory - clean PostgreSQL initialization script
-- Based on the previous migrated dump.
--
-- Important design decisions:
--   1. All application DateTime columns use timestamptz.
--   2. Session timezone is UTC.
--   3. Integer IDs use GENERATED BY DEFAULT AS IDENTITY.
--      This allows importing existing IDs and generating new IDs
--      automatically for new rows.
--   4. SQL Server bit/boolean fields are PostgreSQL boolean.
--   5. Primary/foreign keys are created after data import.
--   6. Identity sequences are synchronized after the import.
--
-- This file is intended to be executed while connected to the
-- already-created "ChemicalLaboratory" database.
-- ============================================================

SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;

-- Все timestamp with time zone в приложении считаем UTC.
SET TIME ZONE 'UTC';

-- public создаётся PostgreSQL автоматически, но IF NOT EXISTS
-- делает скрипт безопаснее.
...
--CREATE DATABASE "chemical_laboratory" WITH TEMPLATE = template0 ENCODING = 'UTF8' LOCALE_PROVIDER = libc LOCALE = 'ru-RU';
...
CREATE TABLE public.notification_settings (
    id integer GENERATED BY DEFAULT AS IDENTITY,
    low_quantity_threshold numeric(10,2),
...
CREATE TABLE public.reagent_operations (
    id integer GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE public.reagent_receipts (
    id integer GENERATED BY DEFAULT AS IDENTITY,
    reagent_id integer NOT NULL,
...
-- ------------------------------------------------------------
-- Data
-- Existing date/time literals are interpreted as UTC because
-- the session timezone is explicitly UTC above.
-- ------------------------------------------------------------

COPY public.work_schedule (id, work_shift, start_time, end_time) FROM stdin;
1	Утренняя	08:00:00	16:00:00
2	Вечерняя	16:00:00	00:00:00
3	Ночная	00:00:00	08:00:00
\.

COPY public.system_roles (id, name, display_name) FROM stdin;
1	User	Пользователь
2	Admin	Администратор
3	Super_Admin	Супер-администратор
\.

COPY public.reagent_categories (id, name, description, is_active, deleted_at) FROM stdin;
...
COPY public.suppliers (id, name, contact_info, address, is_active, deleted_at) FROM stdin;
...
COPY public.operation_types (id, code, name, affects_quantity, is_active) FROM stdin;
...
COPY public.notification_settings (id, low_quantity_threshold, expiration_days_threshold, analytics_update_interval, email_template) FROM stdin;
1	10.00	30	24	Уведомление: реагент {ReagentName} требует внимания
\.

COPY public.reagents (id, name, chemical_formula, unit, current_quantity, min_quantity, expiration_date, storage_location, category_id, created_at, is_active, deleted_at) FROM stdin;
...
COPY public.users (id, work_schedule_id, first_name, middle_name, last_name, email, sex, system_role_id, job_position, login, password_hash, is_active, deleted_at) FROM stdin;
...
COPY public.reagent_receipts (id, reagent_id, supplier_id, quantity, receipt_date, document_number) FROM stdin;
1	1	1	10.00	2025-01-10	INV-001
2	3	2	5.00	2025-01-15	INV-002
3	4	1	20.00	2025-02-01	INV-003
\.

COPY public.reagent_operations (id, reagent_id, user_id, operation_type_id, quantity, operation_date, comment) FROM stdin;
...
COPY public.notifications (id, reagent_id, notification_type, message, created_at, is_read, title, user_id, deleted_at, file_path) FROM stdin;
...
50	\N	Low_Quantity	Сформирована накладная на закупку.	2026-05-22 17:01:49.36	true	Обнаружен дефицит реагентов	8	\N	/invoices/invoice_low_stock_20260522_200149.pdf
...
ALTER TABLE public.operation_types
    ADD CONSTRAINT pk_operation_types PRIMARY KEY (id);

ALTER TABLE public.reagent_categories
    ADD CONSTRAINT pk_reagent_categories PRIMARY KEY (id);

ALTER TABLE public.reagent_operations
    ADD CONSTRAINT pk_reagent_operations PRIMARY KEY (id);

ALTER TABLE public.reagent_receipts
    ADD CONSTRAINT pk_reagent_receipts PRIMARY KEY (id);

ALTER TABLE public.reagents
    ADD CONSTRAINT pk_reagents PRIMARY KEY (id);

ALTER TABLE public.suppliers
    ADD CONSTRAINT pk_suppliers PRIMARY KEY (id);

ALTER TABLE public.system_roles
    ADD CONSTRAINT pk_system_roles PRIMARY KEY (id);

ALTER TABLE public.users
    ADD CONSTRAINT pk_users PRIMARY KEY (id);

ALTER TABLE public.work_schedule
    ADD CONSTRAINT pk_work_schedule PRIMARY KEY (id);
...
ALTER TABLE public.notifications
    ADD CONSTRAINT fk_notifications_reagent
    FOREIGN KEY (reagent_id) REFERENCES public.reagents(id);
...
ALTER TABLE public.reagent_operations
    ADD CONSTRAINT fk_reagent_operations_reagent
    FOREIGN KEY (reagent_id) REFERENCES public.reagents(id);

ALTER TABLE public.reagent_operations
    ADD CONSTRAINT fk_reagent_operations_user
    FOREIGN KEY (user_id) REFERENCES public.users(id);

ALTER TABLE public.reagent_operations
    ADD CONSTRAINT fk_reagent_operations_operationtype
    FOREIGN KEY (operation_type_id) REFERENCES public.operation_types(id);

ALTER TABLE public.reagent_receipts
    ADD CONSTRAINT fk_reagent_receipts_reagent
    FOREIGN KEY (reagent_id) REFERENCES public.reagents(id);

ALTER TABLE public.reagent_receipts
    ADD CONSTRAINT fk_reagent_receipts_supplier
    FOREIGN KEY (supplier_id) REFERENCES public.suppliers(id);

ALTER TABLE public.reagents
    ADD CONSTRAINT fk_reagents_category
    FOREIGN KEY (category_id) REFERENCES public.reagent_categories(id);
...
ALTER TABLE public.users
    ADD CONSTRAINT fk_users_systemrole
    FOREIGN KEY (system_role_id) REFERENCES public.system_roles(id);

-- ------------------------------------------------------------
-- Useful indexes for FK/filter columns
-- ------------------------------------------------------------

CREATE INDEX ix_notifications_reagent_id
    ON public.notifications (reagent_id);

CREATE INDEX ix_notifications_user_id
    ON public.notifications (user_id);

CREATE INDEX ix_reagent_operations_reagent_id
    ON public.reagent_operations (reagent_id);

CREATE INDEX ix_reagent_operations_user_id
    ON public.reagent_operations (user_id);

CREATE INDEX ix_reagent_operations_operation_type_id
    ON public.reagent_operations (operation_type_id);

CREATE INDEX ix_reagent_operations_operation_date
    ON public.reagent_operations (operation_date);

CREATE INDEX ix_reagent_receipts_reagent_id
    ON public.reagent_receipts (reagent_id);

CREATE INDEX ix_reagent_receipts_supplier_id
    ON public.reagent_receipts (supplier_id);

CREATE INDEX ix_reagents_category_id
    ON public.reagents (category_id);

CREATE INDEX ix_reagents_expiration_date
    ON public.reagents (expiration_date);

CREATE INDEX ix_reagents_is_active
    ON public.reagents (is_active);

-- ------------------------------------------------------------
-- Synchronize identity sequences after explicit ID import.
--
-- GENERATED BY DEFAULT AS IDENTITY accepts the imported IDs,
-- but the next automatically generated ID must be greater than
-- the current maximum.
-- ------------------------------------------------------------
...
SELECT setval(
    pg_get_serial_sequence('public.notification_settings', 'id'),
    COALESCE((SELECT MAX(id) FROM public.notification_settings), 1),
    (SELECT COUNT(*) > 0 FROM public.notification_settings)
);

SELECT setval(
    pg_get_serial_sequence('public.operation_types', 'id'),
    COALESCE((SELECT MAX(id) FROM public.operation_types), 1),
    (SELECT COUNT(*) > 0 FROM public.operation_types)
);

SELECT setval(
    pg_get_serial_sequence('public.reagent_categories', 'id'),
    COALESCE((SELECT MAX(id) FROM public.reagent_categories), 1),
    (SELECT COUNT(*) > 0 FROM public.reagent_categories)
);

SELECT setval(
    pg_get_serial_sequence('public.reagent_operations', 'id'),
    COALESCE((SELECT MAX(id) FROM public.reagent_operations), 1),
    (SELECT COUNT(*) > 0 FROM public.reagent_operations)
);

SELECT setval(
    pg_get_serial_sequence('public.reagent_receipts', 'id'),
    COALESCE((SELECT MAX(id) FROM public.reagent_receipts), 1),
    (SELECT COUNT(*) > 0 FROM public.reagent_receipts)
);

SELECT setval(
    pg_get_serial_sequence('public.reagents', 'id'),
    COALESCE((SELECT MAX(id) FROM public.reagents), 1),
    (SELECT COUNT(*) > 0 FROM public.reagents)
);

SELECT setval(
    pg_get_serial_sequence('public.suppliers', 'id'),
    COALESCE((SELECT MAX(id) FROM public.suppliers), 1),
    (SELECT COUNT(*) > 0 FROM public.suppliers)
);

SELECT setval(
    pg_get_serial_sequence('public.system_roles', 'id'),
    COALESCE((SELECT MAX(id) FROM public.system_roles), 1),
    (SELECT COUNT(*) > 0 FROM public.system_roles)
);
...
SELECT setval(
    pg_get_serial_sequence('public.work_schedule', 'id'),
    COALESCE((SELECT MAX(id) FROM public.work_schedule), 1),
    (SELECT COUNT(*) > 0 FROM public.work_schedule)
);